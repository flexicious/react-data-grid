...
export const ToolbarLeft: FC<RendererProps> = ({ node }) => {
    ...
    const isMultiple = node.gridOptions.selectionMode === GridSelectionMode.MultipleRows ||node.gridOptions.selectionMode === GridSelectionMode.MultipleCells || !node.gridOptions.selectionMode;
    return <div className="ezgrid-dg-toolbar-section" >
        ...
        {options?.addFormulaColumnRenderer && (options.addFormulaColumnRenderer({ node }) as ReactNode)}
        ...
        {(options?.enableSelectionCount !== false && (isMultiple) ) && <Selection node={node} /> }
