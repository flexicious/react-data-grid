...
export const MultiSelectFilter: FC<PopupFilterRendererProps> = ({ node, filterBuilderMode, onValueChanged, value, popupWidth, popupHeight }) => {
    ...
    const values = currentValue?.join(",");
    const getDataProvider = () => node.columnPosition ? api.getDistinctFilterValues(node.columnPosition.column) : [];
    const dataProviderLength = getDataProvider().length;
        useEffect(() => {
            setLoading(false);
        }, [dataProviderLength]);
    useEffect(() => {
        if (node.columnPosition?.column) {
            const dp = api.getDistinctFilterValues(node.columnPosition.column) as NameValue[];
...
export const createMultiSelectFilterOptions = ({ popupWidth, popupHeight }: { popupWidth?: number, popupHeight?: number } = {}): FilterOptions => {
    return {
        filterRenderer: (props: RendererProps) => <MultiSelectFilter key={props.node.key} {...props} popupHeight={popupHeight} popupWidth={popupWidth} />,
