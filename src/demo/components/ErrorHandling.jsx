...
export default class ErrorHandling extends React.Component {
  ...
  validate(editor) {
    ...
    if (txt.getValue().length < 3) {
      const errMsg = "Legal name must be greater than 3 characters";
      UIUtils.showMessage(errMsg);
      grid.setErrorByObject(cell.getRowInfo().getData(), cell.getColumn().getDataField(), errMsg);
    } else {
    ...
    return (
      <div>
        ...
        <FullWidthSection useContent={true}>
          <ReactDataGrid width={"100%"} dataProvider={FlexiciousMockGenerator.instance().getFlatOrgList()} ref={(grid) => { this.grid = grid; } } editable enableFooters horizontalScrollPolicy="off">
